...
test  <- CD[ind==2,]
set.seed(213)
rf <- randomForest(factor(highsales)~., data=train)
...
confusionMatrix(pred1, factor(train$highsales))   # 100 % accuracy on training data 
...
confusionMatrix(pred2, factor(test$highsales)) # 80.87 % accuracy on test data

#Error rate in random forest model
plot(rf)

# Tune Random Forest Model mtry 
tune <- tuneRF(train[,-11], factor(train[,11]), stepFactor = 0.5, plot = TRUE, ntreeTry = 300,
               trace = TRUE, improve = 0.05)

rf1 <- randomForest(factor(highsales)~., data=train, ntree = 300, mtry = 3, importance = TRUE,
                                   proximity = TRUE)
rf1               
...
confusionMatrix(pred2, factor(test$highsales)) # 84.35 % accuracy on test data 
...
MDSplot(rf1, factor(CD$highsales))
