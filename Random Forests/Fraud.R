...
test  <- FC[ind==2,]
set.seed(213)
rf <- randomForest(factor(Risky_Good) ~., data=train)
...
confusionMatrix(pred1, factor(train$Risky_Good))
...
confusionMatrix(pred2, factor(test$Risky_Good)) # 100 % accuracy on test data 
...
factortrain1 <-factor(train[,6])
View(factortrain1)
??tuneRF
tune <- tuneRF(train[,-6], factortrain1, stepFactor = 0.5, plot = TRUE, ntreeTry = 300,
               trace = TRUE, improve = 0.05)

rf1 <- randomForest(factor(Risky_Good)~., data=train, ntree = 200, mtry = 2, importance = TRUE,
                    proximity = TRUE)
rf1
...
MDSplot(rf1, factor(FC$Risky_Good))
